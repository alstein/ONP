...
function preview(img, selection) { 
	...
	$('#thumbnail + div > img').css({ 
		...
		height: Math.round(scaleY * 29) + 'px',
...
{if $large_photo_exists && $thumb_photo_exists}	
	...
	<a href='{$siteroot}/admin/user/image_crop.php?userid={$userid}&act=del' onclick="javascript: return del();"><strong>Delete</strong></a>
{else}	
    <table width="70%" border="0" cellspacing="2" cellpadding="6">
      <tr>
        	<td align="left">
			<img src="{$siteroot}/uploads/user_photo/big_image/{$image}" id="thumbnail" style="float: left; margin-right: 10px;" alt="Create Thumbnail" />
			<div style="float:left; position:relative; overflow:hidden; width:{$thumb_width}px; height:{$thumb_height}px;">
				<img src="{$siteroot}/uploads/user_photo/big_image/{$image}" style="position: relative;" alt="Thumbnail Preview" />
			</div>
        	</td>
      </tr>
    </table>
