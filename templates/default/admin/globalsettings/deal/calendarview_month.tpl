...
{$result_found}
      {if $result_found neq 1}
				...
				{section name=k loop=$finals}
					<a href="{$siteroot}/admin/globalsettings/deal/view_product.php?id={$finals[k].product_id}&act=view" class="newone1"><font style="color:#111;">{$finals[k].product_name|substr:0:9}</font>

{if $finals[k].deal_type eq 1}-<font style="color:#1D2EEF;">(main deal)</font>{else}-<font style="color:#DF0000;">(side deal)</font>{/if}</a>
{if $smarty.section.i.iteration gt 1}<br>{/if}
