...
$(".image_stack").delegate('img', 'mouseenter', function() {//when user hover mouse on image with div id=stackphotos 
		...
		// the class stackphotos is not really defined in css , it is only assigned to each images in the photo stack to trigger the mouseover effect on  these photos only 
				...
				var childsphoto=jQuery($parent).children();
				$(childsphoto[0]).addClass('rotate1');
...
$(childsphoto[1]).addClass('rotate2');
				$(childsphoto[2]).addClass('rotate3');
				$(childsphoto[0]).css("left","150px"); // reposition the first and last image 
				$(childsphoto[2]).css("left","50px");
		}
	})
	.delegate('img', 'mouseleave', function() {// when user removes cursor from the image stack
	var $parent = $(this).parent();
				var childsphoto=jQuery($parent).children();
$(childsphoto[0]).removeClass('rotate1');// remove the css class that was previously added to make it to its original position
			$(childsphoto[1]).removeClass('rotate2');
			$(childsphoto[2]).removeClass('rotate3');
			$(childsphoto[0]).css("left","");// remove the css property 'left' value from the dom
$(childsphoto[2]).css("left","");
