	...
	if(eregi("\.gif$",$this->thumb_name))
	{
	 $img = ImageCreateFromGif ($this->thumb_name);
		...
		if(isset($max_width) && isset($max_height) && $max_width != 0 && $max_height != 0)
		{
		 //$new_width = $max_width;
		 //$new_height = $max_height;
		 $this->required["height"] = $max_height;
		 $this->required["width"] = $max_width;
		 $this->image_property[0] = $FullImage_width;
		 $this->image_property[1] = $FullImage_height;
				...
				$new_width = $this->image_property[5];
				$new_height = $this->image_property[6];
