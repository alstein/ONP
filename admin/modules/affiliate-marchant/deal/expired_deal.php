...
// 	$tbl = "tbl_deal_affiliate as p, tbl_deal_affiliate_marchant as tm, tbl_dealtype dt ";
//    	$sf = "p.*";
// 	
// 	if(isset($_GET['search']))
// {
// 	$cnd .= " AND (p.iId LIKE '%".$_GET['search']."%' OR p.iMerchantId LIKE '%".$_GET['search']."%' OR dt.dealtype LIKE '%".$_GET['search']."%' OR p.fPrice LIKE '%".$_GET['search']."%' OR p.fRrpPrice LIKE '%".$_GET['search']."%')";
// }
// 
//  	$res = $dbObj->customqry("SELECT ".$sf." FROM tbl_deal_affiliate as p LEFT JOIN tbl_deal_affiliate_marchant as tm ON p.iId=tm.marchant_id LEFT JOIN tbl_dealtype dt ON dt.typeid=p.deal_main_type WHERE ".$cnd." ORDER BY deal_unique_id DESC LIMIT ".$l, "");
...
//    echo "<pre>";print_r($feed);exit;
   ...
   #------------Pagination2-----------------#   
// 	$res = $dbObj->customqry("SELECT ".$sf." FROM tbl_deal_affiliate as p LEFT JOIN tbl_deal_affiliate_marchant as tm ON p.iId=tm.marchant_id LEFT JOIN tbl_dealtype dt ON dt.typeid=p.deal_main_type WHERE ".$cnd." ORDER BY deal_unique_id DESC", "");
   ...
   {
      $firstlink = "expired_deal.php?iMerchantId=".$_GET['iMerchantId'];
      $seperator = '&page=';
   }else
   {
      $firstlink = "expired_deal.php";
      $seperator = '?page=';
   }
   $baselink = $firstlink;
...
$smarty->display(TEMPLATEDIR.'/admin/modules/affiliate-marchant/deal/expired_deal.tpl'); 
?>
