...
$res_userdetails = $dbObj->customqry("SELECT *, count(duc.d_c_id) viewCount FROM `tbl_affiliate_deal_user_count` duc, `tbl_affiliate_deal_count` dc, `tbl_users` tu WHERE duc.user_id = tu.userid AND duc.d_c_id = dc.d_c_id AND dc.deal_unique_id = ".$_GET['id']." group by tu.userid","");
if($res_userdetails != 'n')
{
	while($row_userdetails = @mysql_fetch_assoc($res_userdetails))
	{
...
$smarty->display(TEMPLATEDIR.'/admin/modules/affiliate-marchant/deal/deal_viewers.tpl'); 
$dbObj->Close();
?>
