...
if($_POST['sort_ords']=='Update')
{
	...
	while($row_results = @mysql_fetch_assoc($dbres))
	{
        ...
        $_SESSION['msg']="<span class='success'>Followus logo order has been updated successfully</span>";
...
$l =  $StartRow.','.$newsperpage;
     ...
     $sql="SELECT * FROM tbl_footer_logos where type='$type' order by sort_no LIMIT $l";
...
if($total_pages > 1)
{
    ...
    if($_GET['searchuser']!='')
    {
	    $firstlink = "followus_logo_list.php?searchuser=".$_GET['searchuser'];
	    $seperator = '&page=';
    }
    else 
    {
	  $firstlink = "followus_logo_list.php";
	  $seperator = '?page=';
    }
    $baselink  = $firstlink;
...
if(isset($_SESSION['msg'])){
   ...
   $smarty->display(TEMPLATEDIR . '/admin/modules/logos/followus_logo_list.tpl');
