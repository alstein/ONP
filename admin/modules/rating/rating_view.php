...
$sql="select * from tbl_deal_rating where rating_id=$rating_id";
$resultrating=mysql_query($sql)or die(mysql_error());
$row=mysql_fetch_array($resultrating);
$deal_id=$row['deal_id'];
$user_id=$row['user_id'];
$ratingmark=$row['rating_mark'];
...
if($rs_user !='n')
{
      ...
      #--------------Cancel order--------------#
      $c_ordr= $dbObj->gj("tbl_deal_payment","sum(deal_quantity)as tot_cancel","user_id='{$_GET['userid']}' and cancel_order = 'yes'","","","","","");
...
$smarty->display(TEMPLATEDIR . '/admin/modules/rating/rating_view.tpl');
