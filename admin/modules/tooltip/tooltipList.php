...
#------------Check For access----------#
if(!(in_array("79", $arr_modules_permit)))
{
...
$res=$dbObj->gj("tbl_tooltip", "tooltip_title,description,module_name,status,tooltip_id", "1", "tooltip_id", "", "asc", $l, "");
...
/*-----------------------Pagination Part2--------------------*/
$rs1 =$dbObj->gj("tbl_tooltip", "tooltip_title,description,module_name,status,tooltip_id", "1", "module_name", "", "asc", "", "");
...
if($total_pages > 1)
{
    ...
    if($_GET['searchuser']!='')
    {
	    $firstlink = "tooltipList.php?searchuser=".$_GET['searchuser'];
	    $seperator = '&page=';
    }
    else 
    {
	  $firstlink = "tooltipList.php";
	  $seperator = '?page=';
    }
    $baselink  = $firstlink;
...
$smarty->display(TEMPLATEDIR . '/admin/modules/tooltip/tooltipList.tpl');
