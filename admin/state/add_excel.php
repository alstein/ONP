...
{
	header('Content-disposition: attachment; filename=testStateFile.csv');
	header('Content-type: application/excel');
	readfile('testStateFile.csv');
	die();
}
...
if(isset($_POST["Submit"]))
{
	...
	if(strlen(trim($_FILES['category_file']['name'])) > 0)
	{
		...
		if (!in_array(end(explode(".", strtolower($_FILES['category_file']['name']))), $allowedExtensions))
		{
			...
			header("location:".SITEROOT."/admin/state/state_list.php?contryid=".$_GET['countryid']);
			exit;
		}else
		{
			...
			if(move_uploaded_file($_FILES['category_file']['tmp_name'], $target_path))
			{
				...
				if($highestRow > 2) //is greater than 2 cause first row is with heading like Name, Email
				{
					for ($row = 2; $row <= $highestRow; ++$row)
					{
						...
						$rs=$dbObj->customqry("select * from mast_state where country_id = ".$_GET['countryid']." and state_name = ".$row_name,"");
						if(@mysql_num_rows($rs) == 0)
						{
							if(strlen(trim($row_name)) > 0)
							{
								$rs=$dbObj->cgi("mast_state",array("country_id", "state_name", "active"),array($_GET['countryid'],$row_name, "1"),"");
...
$smarty->display(TEMPLATEDIR . '/admin/state/add_excel.tpl');
