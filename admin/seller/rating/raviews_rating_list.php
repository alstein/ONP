...
/////////////////////////////////////////////
 ...
 /*-----------------------Pagination Part1--------------------*/
/*----------------------$_GET['deal_id']---Get main deal here and display total rating review--------------------*/
 if($_GET['deal_id'])
 {
if(!isset($_GET['page']))
    $page =1;	
	...
	if(!(@mysql_num_rows($rating_rs) > 0))
        {
            header("location:".SITEROOT . "/admin/seller/rating/raviews_rating_deals_list.php");
            exit;
        }			
	while($row = mysql_fetch_array($rating_rs))
	{
	   $tmp = array(
                ...
                $smarty -> assign("deal_name", $row['deal_title']);
...
$smarty->display(TEMPLATEDIR . '/admin/seller/rating/raviews_rating_list.tpl');
