...
if($_GET['delete']){
		...
		if(in_array($_GET['delete'],$mImage1)){
         ...
         @unlink("../../../uploads/".$removeThis);
			...
			@header("Location:".SITEROOT."/admin/seller/deal/edit_product.php?id=".$_GET['id']);
		}
	}
...
/*
	...
	{
		header("Location:".SITEROOT."/admin/seller/deal/expired_deal.php");
	...
	if($res1 != 'n')
	{
		...
		$dealprice_option = substr($dealprice_option,1);
           ...
           /////////Get category name by using parent id START/////////////
            ...
            /////////Get category name by using parent id end/////////////
...
/*
		...
		$initialValue = stripslashes(html_entity_decode($row3['subtitle']));
...
$res_country=$dbObj->gj("mast_country c",$sf,$cnd, $ob, "", "", $l, "");
	...
	$res_dealcountries = $dbObj->cgs("tbl_deal_country","","deal_id",$_GET['id'],"","","");
	...
	$smarty->assign("deal_countries_arr",explode(",",$deal_countries));
...
#-----------------------------------------------------------------------------#
	...
	$res_state = $dbObj->customqry("select * from mast_state ms,mast_country mcou where ms.country_id = mcou.countryid and mcou.status = 'Active' and ms.active=1  and ms.country_id in(".$deal_countries.") order by ms.state_name ASC","");
...
#-----------------------------------------------------------------------------#
    ...
    $_re1 = $dbObj->customqry("select * from mast_city mc,mast_country mcou where mc.country_id = mcou.countryid and mcou.status = 'Active' and mc.status='Active' and state_id in(".$deal_states.") order by mc.city_name ASC","");
...
$smarty->display(TEMPLATEDIR . '/admin/seller/deal/edit_product.tpl');
$dbObj->Close();
?>
