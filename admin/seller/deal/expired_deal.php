...
#-----------------Get Deal Types--------------------#
   ...
   $tbl = "tbl_deal as p";
   $sf = "p.*";
   $cnd = " p.admin_approve = 'yes' and p.admin_review = '1' and p.deal_status = '1' and (p.end_date < '$date') and admin_userid = ".$_SESSION['duAdmId']." ";
...
//    $res = $dbObj->gj($tbl,$sf,$cnd,"","","","", "");
  $res = $dbObj->customqry("SELECT ".$sf." FROM tbl_deal as p LEFT JOIN tbl_users as tu ON p.deal_from_seller_name=tu.userid LEFT JOIN tbl_dealtype dt ON dt.typeid=p.deal_main_type WHERE ".$cnd." ORDER BY deal_unique_id DESC", "");
   ...
   if($search)
      $firstlink = "expired_deal.php?deal_from_seller_name=ssdf&uname={$_GET['uname']}&dltype=".$_GET['dltype'];
   else
      $firstlink = "expired_deal.php?deal_from_seller_name=".$_GET['deal_from_seller_name']."&dltype=".$_GET['dltype'];
   $seperator = '&page=';
   $baselink = $firstlink;
   ...
   $smarty->display(TEMPLATEDIR.'/admin/seller/deal/expired_deal.tpl');
?>
