...
$get_type = urldecode($_GET['type']);
$get_seller_id = urldecode($_GET['seller_id']);
...
$get_pay_subs_id = urldecode($_GET['pay_subs_id']);
...
$fp = @fsockopen($ssl.$web['host'], $web['porift'], $errnum, $errstr, 30);
...
<div align="left"><div align="left"></div></div>s'];
   ...
   if($_POST['txn_type']=="subscr_payment")
   {
	...
	}else //end if($get_type == "exist_dotheupdate") //do the modify subscription here
	{
		...
		//Modify through the paypal at the time recurring subscription
			// check if transaction id(subscriber id) in databse $row	
	...
	}//end else($get_type == "exist_dotheupdate") //do the modify subscription here
