...
if($_POST["Save"])
{
	...
	header("location:". SITEROOT . "/admin/category/subcatimage.php?cat_id=".$_GET['cat_id']);
...
if($_POST["Update"])
{
	...
	if ($_FILES["image"]["error"]== 4)
	{
		...
		$_SESSION['msg']="<span class='success'>Images updated successfully</span>";
		header("location:". SITEROOT . "/admin/category/subcatimage.php?cat_id=".$_GET['cat_id']);exit;
		//echo "<script>window.history.go(-2);</script>";
	}else
	{
		$image = generalfileupload($_FILES['image'],"../../uploads/subcat_image","");
...
$smarty->display(TEMPLATEDIR . '/admin/category/add_subcatimage.tpl');
$dbObj->Close();
?>
