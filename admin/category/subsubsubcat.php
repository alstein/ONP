...
//  $subsubcategory=$msobj->getCategoryname($_GET['cat_id']);
//  $smarty->assign("subsubcatname",$subsubcategory['category']);
//  $smarty->assign("subsubcatid",$subsubcategory['id']);
//  
//  $subcategory=$msobj->getCategoryname($subsubcategory['parent_id']);
//  $smarty->assign("subcatname",$subcategory['category']);
//  $smarty->assign("subcatid",$subcategory['id']);
//  
//  $maincategory=$msobj->getCategoryname($subcategory['parent_id']);
...
if($_GET['cat_id'])
{
	...
	$rs_mCat = $dbObj->customqry("select dc1.*,dc2.category 'subcategory', dc3.category 'maincategory' from mast_deal_category dc1, mast_deal_category dc2, mast_deal_category dc3 where dc1.id = ".$_GET['cat_id']." and dc1.parent_id = dc2.id and dc2.parent_id = dc3.id", "");	
	...
	if($_GET['view'] == 'excel')
	{
		...
		$out .="\n";
		$out .="Sub Category : ".$row_mCat['subcategory'];
		$out .="\n";
		$out .="Sub Sub Category : ".$row_mCat['category'];
		$out .="\n";
		$out .="\n";
		$out .="Sub Sub Sub Categories List";
		$out .="\n";
		$out .="\n";
...
$firstlink = "subsubsubcat.php?cat_id={$_GET['cat_id']}";
$seperator = '&page=';
$baselink  = $firstlink; 
...
$smarty->display(TEMPLATEDIR . '/admin/category/subsubsubcat.tpl');
