...
if(isset($_POST["Submit"]))
{
	...
	if(strlen(trim($_FILES['category_file']['name'])) > 0)
	{
		...
		if (!in_array(end(explode(".", strtolower($_FILES['category_file']['name']))), $allowedExtensions))
		{
			...
			header("location:".SITEROOT."/admin/category/category_list.php?cat_id=".$_GET['cat_id']);
			exit;
		}else
		{
				...
				if($highestRow > 2) //is greater than 2 cause first row is with heading like Name, Email
				{
					for ($row = 2; $row <= $highestRow; ++$row) 
					{
						...
						if(strlen(trim($row_name)) > 0)
						{
							...
							$tv_sch = array($_SESSION['duAdmId'],$row_name,0,date("Y-m-d"),1);
	...
	while (!feof ($fd))
	{
		...
		if($buffer)
		{
			...
			if($cnt==1)
			{
				...
				$set_values = array($_SESSION['duAdmId'],$buffer[$cnt-1],0,date("Y-m-d"),1);
...
$smarty->display(TEMPLATEDIR . '/admin/category/add_cat_excel.tpl');
