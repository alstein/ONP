...
//LEFT JOIN tbl_deal ON tbl_rating.deal_id=tbl_deal.deal_unique_id
         $rating_query = "SELECT * FROM tbl_rating LEFT JOIN tbl_users ON tbl_rating.user_id=tbl_users.userid  WHERE tbl_rating.user_id=".$_GET['userid']." order by rating_date DESC LIMIT ".$l;
...
$rs1= $dbObj->customqry("SELECT * FROM tbl_rating LEFT JOIN tbl_users ON tbl_rating.user_id=tbl_users.userid WHERE user_id=".$_GET['userid']." order by rating_date DESC","");
 ...
 if(isset($_POST['submit']))
 {
      ...
      if($action == "delete")
	{
		...
		$_SESSION['msg']="<span class='".$s['msgtype']."'>".$s['msgtext']."</span>";		
                header("Location: view_review_consumer.php?userid=".$_GET['userid']);
                exit;
	}
}
}else{
...
$smarty->display(TEMPLATEDIR . '/admin/user/view_review_consumer.tpl');
