...
// 	$rs_subscription = $dbObj->customqry("select * from tbl_subscription_package where id ={$row['subscription_pack_id']}", "");
// 	$row_subscription = @mysql_fetch_assoc($rs_subscription);
// 		$row['pack_name'] = $row_subscription['pack_name'];
//                 $row['allow_deals_per_month'] = $row_subscription['allow_deals_per_month'];
//                 $row['pack_price'] = $row_subscription['pack_price'];
//                 $row['cost_per_success_deal'] = $row_subscription['cost_per_success_deal'];
//                 $row['cost_per_success_deal_percent_doller'] = $row_subscription['cost_per_success_deal_percent_doller'];
//                 $row['cost_sms_deal'] = $row_subscription['cost_sms_deal'];
//                 $row['pack_duration'] = $row_subscription['pack_duration'];
//                 $row['status'] = $row_subscription['status'];
...
#--------------END Get subscription data--------------#
 #--------------START Get subscription data--------------#
	...
	$row_subscription = @mysql_fetch_assoc($rs_subscription);
		$row['pack_name'] = $row_subscription['subs_pack_name'];
                $row['allow_deals_per_month'] = $row_subscription['subs_pack_allow_deals_per_month'];
                $row['pack_price'] = $row_subscription['subs_pack_price'];
                $row['cost_per_success_deal'] = $row_subscription['subs_pack_cost_per_success_deal'];
                $row['cost_per_success_deal_percent_doller'] = $row_subscription['subs_pack_cost_per_success_deal_percent_doller'];
                $row['cost_sms_deal'] = $row_subscription['subs_pack_cost_sms_deal'];
                $row['pack_duration'] = $row_subscription['subs_pack_duration'];
               // $row['status'] = $row_subscription['status'];
...
$res_delivery_service_chr = $dbObj->customqry("SELECT * FROM tbl_delivery_service_charges WHERE set_for = 'user' AND user_id = ".$_GET['userid'],"");
...
$smarty->display(TEMPLATEDIR . '/admin/user/seller_view.tpl');
