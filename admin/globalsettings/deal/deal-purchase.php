...
if(isset($_GET['status']))
{
      ...
      if($status =="expire")
	$cnd_test .=" and p.payment_done = 'yes' and p.deal_id=d.deal_unique_id and p.expiry_date < {$date1}";
      elseif($status =="all")
	$cnd_test = $cnd_test;
      else //pending
	$cnd_test .= " and p.payment_done = 'yes' and p.cancel_order = 'no' and  p.expiry_date >= {$date1}";
}
...
$arr=$dbObj->gj($tbl,"p.*,d.title as title1,d.deal_type,u.username",$cnd_test,"p.pay_id","","DESC",$l,""); 
...
if($arr !='n')
{
      while($deal=@mysql_fetch_assoc($arr))
      {
	      $deal_ar[]=$deal;
              ...
              #----------------Get username--------------#
              $tmp_usr= $dbObj->gj("tbl_users","username"," userid={$deal['user_id']} ","","","","","");
	      $tmp_usrnm = @mysql_fetch_assoc($tmp_usr);
	      $deal_ar[$i]['username ']=$tmp_usrnm['username'];
              #----------------End username--------------#
...
/*----------Pagination Part-2--------------*/
$rs2=$dbObj->gj($tbl,"p.*,d.title as title1,d.deal_type,u.username",$cnd_test,"","","","",""); 
...
{
      $firstlink = "deal-purchase.php?uname={$_GET['uname']}";
      $seperator = '&page=';
}
else if($_GET['uname'] == '' &&  $_GET['status']!='')
{
      $firstlink = "deal-purchase.php?status={$_GET['status']}";
      $seperator = '&page=';
}
else if($_GET['uname'] != '' &&  $_GET['status']!='')
{
      $firstlink = "deal-purchase.php?uname={$_GET['uname']}&status={$_GET['status']}";
      $seperator = '&page=';
}
else
{

      $firstlink = "deal-purchase.php";
      $seperator = '?page=';
}
...
$smarty->display(TEMPLATEDIR . '/admin/globalsettings/deal/deal-purchase.tpl');
$dbObj->Close();
?>