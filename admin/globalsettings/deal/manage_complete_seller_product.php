...
#----------End Check For access----------#
	...
	if($cnd_arra)
	   $cnd_imp = implode(" and ", $cnd_arra);
        ...
        if($_GET['dealstatus']== 'not-completed')
            $cnd = "deal_status = 4  and deal_type='product' and seller_id!=1 ";
        elseif($_GET['dealstatus']== 'all')
            $cnd = "deal_status in (3,4 ) and deal_type='product' and seller_id!=1 ";
        else
	    $cnd = "deal_status =3 and deal_type='product' and seller_id!=1 ";
	...
	if($search)
		$firstlink = "manage_complete_seller_product.php?search=" . $_GET['search'];
	else
		$firstlink = "manage_complete_seller_product.php?dealstatus=".$_GET['dealstatus']."&dealtype=".$_GET['dealtype']."&seller=".$_GET['seller'];
	$seperator = '&page=';
	$baselink = $firstlink;
   ...
   #----------Success message=--------------#
	//back session
$_SESSION['backsession']=SITEROOT ."/admin/globalsettings/deal/manage_complete_seller_product.php";

	
	$smarty->display(TEMPLATEDIR.'/admin/globalsettings/deal/manage_complete_seller_product.tpl');
?>
