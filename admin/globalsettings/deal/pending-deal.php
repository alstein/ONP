...
#------------Check For access----------#
if(!(in_array("9", $arr_modules_permit)))
{
	...
	//print_r($_POST);exit;
		...
		$delDlCities = $dbObj->customqry("delete from tbl_deal_city where deal_id IN (".$deal_ids.")","");
         ...
         if($_POST['action'] == "reject")
         {
	       ...
	       header("Location:".SITEROOT."/admin/globalsettings/deal/rejected-deals.php");
	       exit;
         }
      }
      else
...
$date = date("Y-m-d H:i:s");
   ...
   $sf = "p.*";
      ...
      $cnd = "p.admin_approve = 'no' and p.admin_review = '1' and deal_status = '1'";
      //$cnd="1";
  // }
  if($_GET['dltype'])
{
   ...
   while($row = @mysql_fetch_assoc($res))
   {
      $feed[] = $row;
        ...
        if($row['admin_userid'] != 0 )
        {
         ...
         $sql_ad = "Select u.usertypeid, u.first_name as ad_firstname, u.last_name as ad_lastname,username from tbl_users u where userid =".$row['admin_userid'];
         $res_ad = $dbObj->customqry($sql_ad,0);
         $row_ad = @mysql_fetch_assoc($res_ad);
         $feed[$i]['ad_name'] = $row_ad['ad_firstname']." ".$row_ad['ad_lastname'];

	    $feed[$i]['userType'] = (($row_ad['usertypeid'] == '3')?"Seller":"Admin");
...
// }
//   	$cnd= " (d.deal_unique_id LIKE '%".$_GET['search']."%' || d.deal_from_seller_name LIKE '%".$_GET['search']."%' || d.deal_main_type LIKE '%".$_GET['search']."%' ) and d.admin_approve = 'no' and d.admin_review = '1' and d.deal_status = '1'";
// // 	$cnd .= " and u.lang_id=".$_SESSION['adm_lan_id'];
//   	$sf="d.*, dt.typeid as type_id, dt.dealtype as dealType";
//   	$ob="d.deal_status"; 
//   	$rs=$dbObj->gj("tbl_deal d LEFT JOIN tbl_dealtype dt ON d.deal_main_type=dt.typeid",$sf,$cnd, $ob, "", "DESC", $l, "");
...
$smarty->display(TEMPLATEDIR . '/admin/globalsettings/deal/pending_purchases.tpl');
