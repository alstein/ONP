...
header("location:".SITEROOT."/admin/globalsettings/deal/featured_deal.php");
 exit;
...
#------------Check For access----------#
if(!(in_array("38", $arr_modules_permit)))
{
...
if($_POST['submit']=='update')
{
	...
	header("Location:".SITEROOT."/admin/globalsettings/deal/featured_deal.php".$url_param);
	exit;
}
...
#---------END-------------#
  ...
  #--------Pagination1-------------------------#
   ...
   $sf = "p.*";
   $cnd = "p.admin_approve = 'yes' and admin_review = '1' and deal_status = '1' and (start_date <= '$date' and end_date >= '$date') and featured ='1'";
...
//	$res = $dbObj->gj($tbl,$sf,$cnd,"deal_unique_id","","DESC","", "");
	$res = $dbObj->customqry("SELECT ".$sf." FROM tbl_deal as p LEFT JOIN tbl_users as tu ON p.deal_from_seller_name=tu.userid  WHERE ".$cnd." ORDER BY deal_unique_id DESC LIMIT ".$l, "");
...
// $baselink  = $firstlink; 
if($search)
      $firstlink = "featured_deal.php?deal_from_seller_name=ssdf&uname={$_GET['uname']}&dltype=".$_GET['dltype'];
   else
      $firstlink = "featured_deal.php?deal_from_seller_name=".$_GET['deal_from_seller_name']."&dltype=".$_GET['dltype'];
   $seperator = '&page=';
   $baselink = $firstlink;
...
$smarty->display(TEMPLATEDIR . '/admin/globalsettings/deal/feture-deal.tpl');
