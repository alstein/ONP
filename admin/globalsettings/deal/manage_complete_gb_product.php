...
if($_GET['sortby'])
{
    switch($_GET['sortby'])
    {
          case deal_title:
                        ...
                        break;
          case fullname:
                        $ob = "tu.fullname"; $ot = $_GET['sorttype'];
...
$date = date("Y-m-d H:i:s");	
   ...
   $sf = "p.*,tu.fullname,tu.business_name,tu.deal_cat";
...
// 	$res = $dbObj->customqry("SELECT ".$sf." FROM tbl_deals as p LEFT JOIN tbl_users as tu ON p.merchant_id=tu.userid  WHERE ".$cnd." ORDER BY $ob $ot ", "");
	}
	else
	{
	$res = $dbObj->customqry("SELECT ".$sf." FROM tbl_deals as p LEFT JOIN tbl_users as tu ON p.merchant_id=tu.userid left join mast_deal_category md on tu.deal_cat=md.id WHERE ".$cnd." ORDER BY $ob $ot LIMIT ".$l, "");
// 	$res = $dbObj->customqry("SELECT ".$sf." FROM tbl_deals as p LEFT JOIN tbl_users as tu ON p.merchant_id=tu.userid  WHERE ".$cnd." ORDER BY $ob $ot LIMIT ".$l, "");
	}
	$i=0;
	while($row = @mysql_fetch_assoc($res))
	{
		$sel_buy_deal_no=$dbObj->customqry("SELECT count(*) as count FROM tbl_deal_payment_unique  WHERE deal_id='".$row['deal_unique_id']."'", "");
		$res_buy_deal=@mysql_fetch_assoc($sel_buy_deal_no);
		$count_deal_buy=$res_buy_deal['count'];		
		$feed[] = $row;
		$feed[$i]['deal_count']=$count_deal_buy;	
		if($_GET['view'] == 'excel')
		{
...
//echo "<pre>";print_r($feed);echo "</pre>";
   ...
   $res = $dbObj->customqry("SELECT ".$sf." FROM tbl_deals as p LEFT JOIN tbl_users as tu ON p.merchant_id=tu.userid  WHERE ".$cnd." ORDER BY deal_unique_id DESC ", "");
   ...
   #----------Success message=--------------#
	...
	#----code end------#
   $smarty->display(TEMPLATEDIR.'/admin/globalsettings/deal/manage_complete_gb_product.tpl'); 
?>
