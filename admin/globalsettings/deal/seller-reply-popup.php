...
// 	$temp = $dbObj->customqry("update tbl_deal set admin_approve = 'yes', deal_status = 2, reject_by_id=".$_SESSION['duAdmId']." where deal_unique_id=".$_GET['id'], "");
	$_SESSION['msg']="<span class='success'>Message sent to seller.</span>";
       

         #--fetching email content--#
                $mail_rs= $dbObj->cgs("mast_emails","*",array("emailid"),array(24),"","","");
                $mail = @mysql_fetch_assoc($mail_rs);    
                $mail_content=stripslashes(html_entity_decode($mail['message']));
                
                #--end--#     
            $title="Response to your comment for - ".$row['title'];
...
//            echo   $message; exit; 
            ...
            $flag=@mail("k.varad@agiletechnosys.com",$title,$message,$headers);    
...
$smarty->display( TEMPLATEDIR.'/admin/globalsettings/deal/seller-reply-popup.tpl');
