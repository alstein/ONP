...
define(TABLENAME1,'messages me, user us ,outbox ob');
	...
	$enc_get_to_id = $_GET['to_id'];
	$get_to_id = (int)str_replace("SC","=",base64_decode(base64_decode(urldecode(urldecode($enc_get_to_id)))));
	if($get_to_id >0)
	{
	...
	if(isset($_POST['message']) ||  !empty($_FILES))
	{
		...
		$to_id = (int)str_replace("SC","=",base64_decode(base64_decode(urldecode(urldecode($to_id)))));
