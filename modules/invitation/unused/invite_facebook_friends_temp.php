...
//                                              echo "<br/>he".$p;
												...
												//$facebook->api_client->stream_publish($post);
												//$res[] = $facebook->api($location, 'POST', $post);
												$facebook->api('/me/feed?uqid='.time(), 'POST', $post);
												echo "===>4";
	                                           //  $res[]="https://graph.facebook.com/".$sendTo."/feed?access_token=".$access_token;
												
											echo "==>".	$_SESSION['connect_msg']="Your message is posted on friend's wall";
											//	echo "===>5";exit;
									}
// 									PRINT_R($res);EXIT;
...
//echo "<pre>";print_r($friends['data']);echo "</pre>";exit;
for($i=0;$i<$frdcnt;$i++){
	echo '<TR><td style="width:25px;"></td><td style="width:25px;padding-bottom:10px;"><input type="checkbox" name="sfriends[]" id="sfriends[]" value='.$friends['data'][$i]['id'].'></td><TD style="width:300px;align:left;vertical-align:top;padding-bottom:10px;">';
