...
{
	$cnd = "username='".trim($_REQUEST['username'])."' and isDeleted=0 and fb_user_id = 0 and twitter_uid = 0 and usertypeid=3";
...
if(trim($_REQUEST['email']) != "")
{
	...
	$cnd = "email='".trim($_REQUEST['email'])."' and isDeleted=0 and usertypeid = 3 and fb_user_id = 0 and twitter_uid = 0";
	if(isset($_REQUEST['userid']) && $_REQUEST['userid']!= null ){
		$cnd .= "  AND userid !=".$_REQUEST['userid'];
	}
	$rs = $dbObj->gj("tbl_users", "email", $cnd, "", "", "", "", "");
