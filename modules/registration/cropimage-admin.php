...
if($_POST['crop_photo'])
{
	...
	if($_POST['x'])
	{
		...
		$src = '../../uploads/merchant_photos/'.$_SESSION['imag'];		
		...
		if (!$function = $functions[$size['mime']]) 
		{
			...
			imagejpeg($dst_r, "../../uploads/merchant_photos/thumb588X288/".$target_file, $jpeg_quality);
			imagedestroy($dst_r);
			...
			imagecopyresampled($dst_r, $img_r, 0, 0, $_POST['x'], $_POST['y'], $targ_w, $targ_h, $_POST['w'], $_POST['h']);
			imagejpeg($dst_r, "../../uploads/merchant_photos/thumb76X64/".$target_file, $jpeg_quality);
			imagedestroy($dst_r);
			...
			imagecopyresampled($dst_r, $img_r, 0, 0, $_POST['x'], $_POST['y'], $targ_w, $targ_h, $_POST['w'], $_POST['h']);
			imagejpeg($dst_r, "../../uploads/merchant_photos/thumb332X290/".$target_file, $jpeg_quality);
			imagedestroy($dst_r);	
//	  		 $dbObj->cupdt("tbl_users", array('thumbnail','bigcrop'), array($target_file,$target_file), "userid", $_SESSION['csUserId'], 0); 
	...
	function frm_submit(){
        ...
        <form action="<?php echo SITEROOT ?>/modules/registration/merchant_reg_profilepic.php" method="POST" name="frm" id="frm">
            <input type="hidden" name="box" id="box" value="box" />
            <b>Image uploaded Successfully!<br /> If you want to add more image <a  href="uploadmain-admin.php">click here</a> else <a onclick="javascript:frm_submit();" href="javascript:void(0)" id="c" >continue</a>.
        </form>
	<script language="JavaScript" type="text/javascript">
	function closena(){
                alert("sdf");
...
//    $smarty->display(TEMPLATEDIR . '/modules/my-account/add-deal.tpl');
    $dbObj->Close();
?>
<script src= "<? echo SITEROOT;?>/js/jquery-1.2.6.pack.js"></script>
<script src="<? echo SITEROOT;?>/js/jquery.Jcrop.js"></script>
...
<div align="center"><?=$msg_success;?></div> 
	<tr>
		<td>
		<FORM action="" method="post" id="frmCrop" name="frmCrop"  enctype="multipart/form-data" >
			
			
		<div style="overflow:auto;width:600px;height:400px">
			<img src="<? echo SITEROOT;?>/uploads/merchant_photos/<?=$_SESSION['imag'];?>" id="cropbox">
		</div>
			<br>
