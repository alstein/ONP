...
if($_GET['moduleid']=='review') {
    if($_GET['status']=='new') {
        ...
        $select_activity=$dbObj->customqry("select f.*,r.*,u.userid,u.business_name,u.usertypeid,u.first_name,u.last_name,u.photo from tbl_users u left join tbl_rating r on r.user_id = u.userid left join tbl_friends f on u.userid = f.userid Where r.merchant_id='".$_GET['userid']."' order by f.userid DESC, r.rating_id  limit $l","");
//        echo "select f.*,r.*,u.userid,u.business_name,u.usertypeid,u.first_name,u.last_name,u.photo from tbl_users u left join tbl_rating r on r.user_id = u.userid left join tbl_friends f on u.userid = f.userid Where r.merchant_id='".$_GET['userid']."' order by f.userid, r.rating_id DESC limit $l";
        $res_all=$dbObj->customqry("select r.*,u.business_name,u.usertypeid,u.first_name,u.last_name,u.photo from tbl_rating r left join tbl_users u on r.user_id =u.userid  where r.merchant_id ='".$_GET['userid']."'  order by rating_id  DESC ","");
    }
